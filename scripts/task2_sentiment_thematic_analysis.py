...
# --- UPDATE DATABASE (WITHOUT REPLACING TABLE) ---
with engine.connect() as conn:
    for idx, row in df.iterrows():
        stmt = text("""
            ...
            WHERE review_id = :rid
        """)
        conn.execute(stmt, {"label": row['sentiment_label'], "score": row['sentiment_score'], "rid": row['review_id']})
print("Sentiment analysis updated successfully in the database.")
...
# Extract top 10 keywords/themes per bank
themes = {}
...
# Display top keywords/themes per bank
